...
class Admin extends CI_Controller
{
	...
	// public function cekRole($role_diizinkan){
	// 	$this->load->library('session');
	// 	$role_pengguna = $this->session->userdata('role');
	// 	if ($role_pengguna != $role_diizinkan) {
	// 		redirect('error_page'); // Atau tampilkan pesan error
	// 	}
	// }
	...
	public function index(){
		...
		$data['totalkelas'] = $this->Admin_models->ambilTotalData('tbl_kelas');
		$data['data'] = $this->Admin_models->ambilData('tbl_admin');
		...
		if ($this->input->post('manajemen') !== NULL) {
			...
			if($this->input->post('manajemen') == 'tambah'){
				...
				$values = array_column($this->input->post('data'),'value');
				 // Gabungkan keys dan values menjadi array asosiatif
				 $dataAssoc = array_combine($keys, $values);

				//  Hash MD5
				 if (isset($dataAssoc['password']) && !empty($dataAssoc['password'])) {
					$dataAssoc['password'] = md5($dataAssoc['password']);
				}

				$tambah = $this->Admin_models->setTambah('tbl_'.$form,$dataAssoc);
			...
			if($this->input->post('manajemen') == 'update'){
				if($form == 'jadwal'){
					...
					// Gabungkan keys dan values menjadi array asosiatif
					$dataAssoc = ['status'=>"Aktif"];
					$where = array('id'=>$id);
					$this->db->query("update tbl_jadwal set status = 'Non Aktif'");
					$update = $this->Admin_models->setUpdate($dataAssoc, $where, 'tbl_' . $form);
	...
	public function generateJam(){
		...
		$jeda = new DateInterval('PT5M'); // Interval jeda 5 menit
		...
		if ($jam_mulai < $jam_selesai) {
			...
			$res = array(false, 'Jam mulai harus lebih kecil dari jam selesai');
		}
	...
	public function pengampu(){
		if ($this->input->post('manajemen') !== NULL) {
			...
			$this->cekLogin();
			$data['data'] = $this->Admin_models->query("SELECT *, (SELECT nama FROM tbl_matakuliah WHERE id = id_mk) as matakuliah, (SELECT semester FROM tbl_matakuliah WHERE id = id_mk) as semester, (SELECT nama FROM tbl_dosen WHERE id = id_dosen) as dosen FROM tbl_pengampu");
	...
	public function buatjadwal(){
		...
		$data['data'] = $this->Admin_models->ambilDataJadwal('tbl_jadwalkuliah');
		$data['jadwal'] = $this->Admin_models->ambilDataJadwals('tbl_jadwal');
		$data['dataPengampu'] = $this->Admin_models->ambilDataPengampu();
			...
			for($i = 0;$i < $jumlah_generasi;$i++ ){
				...
				for ($j = 0; $j < count($fitnessAfterMutation); $j++){
			 	...
			 	// 	$response['fitness'] = number_format($fitnessAfterMutation[$j], 4);
					...
					if($fitnessAfterMutation[$j] >= 0.1){								
						$this->db->query("insert into tbl_jadwal (name) values ('".$this->input->get('name')."') ");
						$ids = $this->db->query("select id from tbl_jadwal order by id desc limit 1 ")->result_array()[0]['id'];
						...
						for($k = 0; $k < count($jadwal_kuliah);$k++){
							...
							$this->db->query("INSERT INTO tbl_jadwalkuliah(id_pengampu,id_jam,id_hari,id_ruang,kelas,id_jadwal) ".
											 "VALUES($id_pengampu,$id_jam,$id_hari,$id_ruang,'$kelas','$ids')");
						}
